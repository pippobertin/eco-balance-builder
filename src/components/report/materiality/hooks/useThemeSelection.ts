...
export const useThemeSelection = ({
  ...
  useEffect(() => {
    ...
    if (lastOpRef.current && Date.now() - lastOpRef.current.timestamp < 5000) {
      console.log(`useThemeSelection [${tabId}]: Skipping update due to recent operation for: ${lastOpRef.current.id}`);
      return;
    }
    
    // Prevent concurrent updates while one is in progress
    if (updatingRef.current) {
      console.log(`useThemeSelection [${tabId}]: Skipping update due to another update in progress`);
      return;
    }
    
    // Log for debugging
    console.log(`useThemeSelection [${tabId}]: Effect running, issues length=${issues.length}, selectedIds=${Array.from(selectedIssueIds).join(',')}`);
    ...
    if (idsChanged || !hasMountedRef.current || availableIssues.length === 0 || selectedIssues.length === 0) {
      ...
      issues.forEach(issue => {
        ...
        if (explicitlyDeselectedRef.current.has(issueCopy.id)) {
          console.log(`useThemeSelection [${tabId}]: Issue was explicitly deselected:`, issueCopy.id);
          issueCopy.isMaterial = false;
          available.push(issueCopy);
          return;
        }
        ...
        if (selectedIssueIds.has(issueCopy.id) || knownMaterialIssuesRef.current.has(issueCopy.id)) {
          console.log(`useThemeSelection [${tabId}]: Issue is selected by ID:`, issueCopy.id);
          // Ensure isMaterial is explicitly true (BOOLEAN)
          issueCopy.isMaterial = true;
          knownMaterialIssuesRef.current.add(issueCopy.id);
          selected.push(issueCopy);
        } else {
          console.log(`useThemeSelection [${tabId}]: Issue is not material:`, issueCopy.id);
      ...
      console.log(`useThemeSelection [${tabId}]: Setting available issues:`, available.length);
      console.log(`useThemeSelection [${tabId}]: Setting selected issues:`, selected.length);
  ...
  const handleIssueSelect = (issue: MaterialityIssue) => {
    ...
    console.log(`useThemeSelection [${tabId}] handling issue select:`, issueToUpdate.id, "isMaterial:", issueToUpdate.isMaterial);
    ...
    if (isSelecting) {
      ...
      console.log(`useThemeSelection [${tabId}]: Moving issue to selected panel:`, issueToUpdate.id);
      ...
      console.log(`useThemeSelection [${tabId}]: Moving issue to available panel:`, issueToUpdate.id);
    ...
    setTimeout(() => {
      console.log(`useThemeSelection [${tabId}]: Passing issue to parent handler:`, issueToUpdate.id, "isMaterial:", issueToUpdate.isMaterial);
