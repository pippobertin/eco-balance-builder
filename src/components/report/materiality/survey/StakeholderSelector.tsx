...
const StakeholderSelector: React.FC<StakeholderSelectorProps> = ({
  ...
  const { pending, sent, completed } = stakeholderGroups;
  const allStakeholders = [...pending, ...(forceResend ? [...sent, ...completed] : [])];
  const isAllSelected = allStakeholders.length > 0 && 
    allStakeholders.every(s => selectedStakeholders.includes(s.id));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
        <div className="flex items-center gap-2">
          ...
          <Switch 
            id="force-resend" 
            checked={forceResend} 
            onCheckedChange={onToggleForceResend} 
          />
        </div>
            ...
            {pending.map((stakeholder) => (
              <div key={stakeholder.id} className="flex items-center space-x-2">
                ...
                <Label htmlFor={`stakeholder-${stakeholder.id}`} className="flex items-center">
                  ...
                  <span 
                    className={`ml-2 text-xs px-2 py-0.5 rounded-full ${getStakeholderPriorityColor(stakeholder.priority)}`}
                  >
      ...
      {forceResend && completed.length > 0 && (
        <div className="space-y-2">
          <h4 className="font-medium flex items-center gap-2 text-sm text-gray-500">
