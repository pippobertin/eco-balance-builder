...
const GHGEmissionsCalculator: React.FC<GHGEmissionsCalculatorProps> = ({
  ...
  useEffect(() => {
    ...
    if (newCounts.scope1 !== calculationLogCountRef.current.scope1 || newCounts.scope2 !== calculationLogCountRef.current.scope2 || newCounts.scope3 !== calculationLogCountRef.current.scope3) {
  ...
  const getActiveTabContent = () => {
    switch (activeTab) {
      case 'scope1':
        return <Scope1Form scope1Source={typeof inputs.scope1Source === 'string' ? inputs.scope1Source : 'fuel'} setScope1Source={value => updateInput('scope1Source', value)} fuelType={typeof inputs.fuelType === 'string' ? inputs.fuelType as FuelType : 'DIESEL'} setFuelType={value => updateInput('fuelType', value)} fuelQuantity={typeof inputs.fuelQuantity === 'string' ? inputs.fuelQuantity : ''} setFuelQuantity={value => updateInput('fuelQuantity', value)} fuelUnit={typeof inputs.fuelUnit === 'string' ? inputs.fuelUnit : 'L'} setFuelUnit={value => updateInput('fuelUnit', value)} periodType={typeof inputs.periodType === 'string' ? inputs.periodType as PeriodType : PeriodType.ANNUAL} setPeriodType={value => updateInput('periodType', value)} />;
      case 'scope2':
        return <Scope2Form energyType={typeof inputs.energyType === 'string' ? inputs.energyType as EnergyType : 'ELECTRICITY_IT'} setEnergyType={value => updateInput('energyType', value)} energyQuantity={typeof inputs.energyQuantity === 'string' ? inputs.energyQuantity : ''} setEnergyQuantity={value => updateInput('energyQuantity', value)} renewablePercentage={typeof inputs.renewablePercentage === 'number' ? inputs.renewablePercentage : 0} setRenewablePercentage={value => updateInput('renewablePercentage', value)} periodType={typeof inputs.periodType === 'string' ? inputs.periodType as PeriodType : PeriodType.ANNUAL} setPeriodType={value => updateInput('periodType', value)} energyProvider={typeof inputs.energyProvider === 'string' ? inputs.energyProvider : ''} setEnergyProvider={value => updateInput('energyProvider', value)} />;
      case 'scope3':
        return <Scope3Form scope3Category={typeof inputs.scope3Category === 'string' ? inputs.scope3Category : 'transport'} setScope3Category={value => updateInput('scope3Category', value)} transportType={typeof inputs.transportType === 'string' ? inputs.transportType as TransportType : 'BUSINESS_TRAVEL_CAR'} setTransportType={value => updateInput('transportType', value)} transportDistance={typeof inputs.transportDistance === 'string' ? inputs.transportDistance : ''} setTransportDistance={value => updateInput('transportDistance', value)} wasteType={typeof inputs.wasteType === 'string' ? inputs.wasteType as WasteType : 'WASTE_LANDFILL'} setWasteType={value => updateInput('wasteType', value)} wasteQuantity={typeof inputs.wasteQuantity === 'string' ? inputs.wasteQuantity : ''} setWasteQuantity={value => updateInput('wasteQuantity', value)} purchaseType={typeof inputs.purchaseType === 'string' ? inputs.purchaseType as PurchaseType : 'PURCHASED_GOODS'} setPurchaseType={value => updateInput('purchaseType', value)} purchaseQuantity={typeof inputs.purchaseQuantity === 'string' ? inputs.purchaseQuantity : ''} setPurchaseQuantity={value => updateInput('purchaseQuantity', value)} purchaseDescription={typeof inputs.purchaseDescription === 'string' ? inputs.purchaseDescription : ''} setPurchaseDescription={value => updateInput('purchaseDescription', value)} periodType={typeof inputs.periodType === 'string' ? inputs.periodType as PeriodType : PeriodType.ANNUAL} setPeriodType={value => updateInput('periodType', value)} vehicleType={typeof inputs.vehicleType === 'string' ? inputs.vehicleType : ''} setVehicleType={value => updateInput('vehicleType', value)} vehicleFuelType={typeof inputs.vehicleFuelType === 'string' ? inputs.vehicleFuelType as FuelType : 'DIESEL'} setVehicleFuelType={value => updateInput('vehicleFuelType', value)} vehicleEnergyClass={typeof inputs.vehicleEnergyClass === 'string' ? inputs.vehicleEnergyClass : ''} setVehicleEnergyClass={value => updateInput('vehicleEnergyClass', value)} vehicleFuelConsumption={typeof inputs.vehicleFuelConsumption === 'string' ? inputs.vehicleFuelConsumption : ''} setVehicleFuelConsumption={value => updateInput('vehicleFuelConsumption', value)} vehicleFuelConsumptionUnit={typeof inputs.vehicleFuelConsumptionUnit === 'string' ? inputs.vehicleFuelConsumptionUnit : 'l_100km'} setVehicleFuelConsumptionUnit={value => updateInput('vehicleFuelConsumptionUnit', value)} />;
      default:
        return null;
    }
  };
  ...
  return <div className="border rounded-md p-4 bg-white/80">
      ...
      {editMode && <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
      ...
      <div className="mt-8 space-y-6">
        ...
        {(!calculationLogs || !calculationLogs.scope1Calculations?.length && !calculationLogs.scope2Calculations?.length && !calculationLogs.scope3Calculations?.length) && <p className="text-gray-500 text-sm">Nessun calcolo effettuato. Utilizzare il calcolatore per aggiungere emissioni.</p>}
        
        {calculationLogs?.scope1Calculations && calculationLogs.scope1Calculations.length > 0 && <EmissionsCalculationTable scope="scope1" scopeLabel="Scope 1 - Emissioni Dirette" calculations={calculationLogs.scope1Calculations} onRemoveCalculation={handleRemoveCalculation} onEditCalculation={handleEditCalculation} />}
        
        {calculationLogs?.scope2Calculations && calculationLogs.scope2Calculations.length > 0 && <EmissionsCalculationTable scope="scope2" scopeLabel="Scope 2 - Energia" calculations={calculationLogs.scope2Calculations} onRemoveCalculation={handleRemoveCalculation} onEditCalculation={handleEditCalculation} />}
        
        {calculationLogs?.scope3Calculations && calculationLogs.scope3Calculations.length > 0 && <EmissionsCalculationTable scope="scope3" scopeLabel="Scope 3 - Altre Emissioni" calculations={calculationLogs.scope3Calculations} onRemoveCalculation={handleRemoveCalculation} onEditCalculation={handleEditCalculation} />}
      </div>
    </div>;
};
