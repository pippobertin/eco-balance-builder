...
const Footer = () => {
  ...
  const footerSections = [
    {
      ...
      links: [
        ...
        { name: 'Report Generator', path: '/report' },
        { name: 'ESG Guidelines', path: '/guidelines' },
        { name: 'V-SME Standard', path: '/standard' }
      ]
    },
    {
      title: 'Company',
  ...
  return (
    <footer className="bg-esg-gray">
      <div className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div>
            <Link to="/" className="flex items-center space-x-2">
              <div className="w-10 h-10 bg-esg-blue rounded-lg flex items-center justify-center">
            ...
            >
              ...
              <ul className="space-y-2">
                {section.links.map((link, i) => (
                  <motion.li key={i} variants={itemAnimation}>
                    <Link 
                      to={link.path} 
                      className="text-esg-gray-medium text-sm hover:text-esg-blue transition-colors"
                    >
                      {link.name}
                    </Link>
                  </motion.li>
